...
```{r}
d %>% filter(!is.na(Game2Angry1)) %>% select(Game1Angry1) %>% range # use range

#d$Game2Angry1 # look at all values directly

# d %>% ggplot(aes(x=Game2Angry1)) + geom_histogram(binwidth = 1) # alternative histogram code

hist(d$Game2Angry1,breaks=c(0.5,1.5,2.5,3.5,4.5,5.5,6.5,7.5,8.5)) # use histogram
```
...
performance_hyp_d = filtered_d %>%  
  # The instructions say that you don't need to use a `filter`, but since this is the performance hypothesis, it makes more sense to filter out the subjects with unusable performance data
  filter(is.na(DoNotUseVideoGamePerformanceData)) %>% 
  # additionally, if I don't get rid of the DoNotUseVideoGamePerformanceData column, I get an error when I try to compute the ImprovementScore
  select( -DoNotUseVideoGamePerformanceData, -starts_with("Game") )
  
  # If I just wanted to get rid of the "Game" columns, I would use the code below:
...
#performance_hyp_long_d <- performance_hyp_long_d %>% filter(Measurement != "DoNotUseVideoGamePerformanceData")
...
```{r}
# the extra variables SEMImprovementScore, lower and upper are for plotting the SEM error bars
...
performance_diff_summary_d = performance_diff_d %>% 
  ...
  summarise(MeanImprovementScore = mean(ImprovementScore, na.rm=T), 
            SEMImprovementScore = std.error(ImprovementScore, na.rm=T)) %>%
  mutate(lower = MeanImprovementScore - SEMImprovementScore,
        upper = MeanImprovementScore + SEMImprovementScore)
...
```{r TamirFig2}
# for the error bar code, I looked at the documentation here: https://ggplot2.tidyverse.org/reference/geom_linerange.html
dodge <- position_dodge(width=0.9)
ggplot(performance_diff_summary_d, aes(x=ConfrontationalGame, y=MeanImprovementScore, fill=MusicCondition)) +
  geom_bar(position=dodge, stat="identity") + 
  scale_fill_brewer(palette="Set1") + geom_errorbar(aes(ymin=lower,ymax=upper), position = dodge, width=0.25)
